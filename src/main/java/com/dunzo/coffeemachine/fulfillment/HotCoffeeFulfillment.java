...
public class HotCoffeeFulfillment implements BeverageFulfillMent {

    private TotalItemsQuantity totalItemsQuantity;
    private CalculateIngredients calculateIngredients;

    public HotCoffeeFulfillment(TotalItemsQuantity totalItemsQuantity) {
...
//            }).start();
            pourBeverage.pour(Collections.singleton(BeverageType.hotCoffee));
        }

        String message = calculateIngredients.getMessage(BeverageType.hotCoffee, validityResponse);
